...
function canLazyFetchOpenDataPhotos(placeId: string): boolean {
  const [source] = placeId.split(':')
  return ['osm', 'overture', 'wikidata', 'user'].includes(source)
}

function canLazyFetchPlacePhotos(placeId: string): boolean {
    ...
    <div className={`relative border border-border rounded-xl p-3 ${onDelete ? 'pr-9' : ''} ${meta.cardBg}`} data-testid={`rec-${rec.placeId}`}>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {hasPhotoSource ? (
            <PhotoStrip photos={photos} placeId={rec.placeId} placeName={displayName.primary} className="mt-2" emptyFallback={photoFallback} placeType={rec.type} aliases={photoAliases} lat={rec.lat} lng={rec.lng} onPhotoUnavailable={onPhotoUnavailable} previewCount={5} autoFetchKind="all" deferGooglePhotoMedia />
          ) : photoFallback}
