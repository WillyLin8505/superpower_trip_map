...
### DEC-006 - GStack Opens Product Discovery

New product work starts with GStack-style product discovery before engineering planning. The Manager must challenge the problem, UX, assumptions, risks, edge cases, architecture, and acceptance criteria before implementation begins.

### DEC-007 - Superpowers Owns Engineering Source of Truth

After product direction is approved, Superpowers owns the durable engineering specification: implementation spec, technical design, implementation plan, testing strategy, and verification strategy.

### DEC-008 - GSD Owns Project Management

GSD is applied after the engineering plan is stable. Its role is to convert the plan into epics, features, tasks, dependencies, status buckets, task boundaries, review requirements, and safe parallel work planning.

### DEC-009 - Workers Execute, Managers Integrate

Workers execute one well-defined task at a time and produce handoff summaries. The Manager reviews, accepts or rejects work, updates project state, and regenerates the task pool and parallel work plan.

### DEC-010 - GStack Reviews Completed Work

Completed Worker output must be challenged before acceptance when the change is non-trivial. GStack-style review should inspect implementation quality, architecture, UX, maintainability, tests, hidden bugs, and edge cases.

### DEC-011 - Worker Session Skills Are Fixed

Every new Worker session follows the same skill sequence: `$multi-new-session` to initialize from planning state, `$multi-claim-task` to verify and claim one task, Superpowers implement/debug/test workflow to execute, and `$multi-handoff-task` to return a Manager-ready summary.
