...
test('one unlocked before locked: unlocked ends exactly at locked start', () => {
  ...
  const locked = makePlace({ startTime: '11:00', durationMin: 90, travelMinToNext: 0, startLocked: true })
...
test('unlocked after locked: starts at lock end', () => {
  ...
  const locked = makePlace({ startTime: '10:00', durationMin: 60, travelMinToNext: 30, startLocked: true })
...
test('unlocked before and after locked', () => {
  ...
  const locked = makePlace({ startTime: '11:00', durationMin: 90, travelMinToNext: 30, startLocked: true })
...
test('outsideHours true when backwards fill goes before 09:00', () => {
  ...
  const locked = makePlace({ startTime: '09:30', durationMin: 60, travelMinToNext: 0, startLocked: true })
  const result = recalcPlan(makePlan([unlocked, locked]))
...
test('locked place outsideHours recomputed', () => {
  ...
  const locked = makePlace({ startTime: '10:00', durationMin: 60, startLocked: true, openingHours: null, outsideHours: true })
...
test('locked card at 08:00 with null openingHours gets outsideHours true', () => {
  const locked = makePlace({ startTime: '08:00', durationMin: 60, travelMinToNext: 0, startLocked: true, openingHours: null, outsideHours: false })
  const result = recalcPlan(makePlan([locked]))
  expect(result.days[0].places[0].outsideHours).toBe(true)
})
...
test('between-segment overflow past next lock gets outsideHours true', () => {
  const lock1 = makePlace({ startTime: '10:00', durationMin: 60, travelMinToNext: 0, startLocked: true })
  const overflow = makePlace({ durationMin: 90, travelMinToNext: 0, startLocked: false })
  const lock2 = makePlace({ startTime: '11:00', durationMin: 60, travelMinToNext: 0, startLocked: true })
