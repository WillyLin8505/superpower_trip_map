...
> **Lane A 修訂 2（2026-06-28，日期為準）：** #2 行程日曆日期上線後，**天數改以日期為準**（覆寫本文件原「天數＝夜數＋1」假設）。確定模型：
> - **天數 = #2 起訖日的 N 天**（不變）；飯店當地理錨點分配到其中幾天。
> - **飯店指派**：`inferNightOrder` 把 K 間飯店排成夜序後，夜 j 的飯店 → 第 j 天的錨點（第 1…min(K,N) 天有飯店）。
> - **住宿不足提醒**：第 1…N−1 天中**沒有住宿類型卡片**的天，於該天標頭顯示「⚠ 這天沒有住宿」（衍生顯示，不存欄位；最後一天回家可無住宿）。
> - **飯店卡片位置**：每間飯店是**當晚那天的最後一張卡**；隔天路線以該飯店位置為起點參考，但**不重複出卡**。
> - **景點分群**：每景點歸到「最近飯店的那天」（home day），依「離當晚飯店」近→遠排，超過 `DAY_BUDGET` 只溢到隔天（釘住）。
> - **只在初次 `planItinerary`（server）且有 ≥1 住宿時啟用**；無住宿維持現有「按數量切天」。
> - **§3.3 建議停留提醒**：由 `DWELL[type]` 直接推導（卡片端比較 `durationMin < DWELL[type]`），**不新增資料欄位**（#7 有真實估時再存）。
