...
2. **就近 home-night + 累進填滿（核心；使用者指定「不平分」）**
   每天有時間預算上限 `DAY_BUDGET_MIN`（暫定 8 小時 = 480 分；含景點停留，不含交通，PoC 可調）。
   - **先歸夜**：每個景點算到各飯店夜的 haversine，歸到**最近的那一夜（home night）** → 保住地理性。
   - **累進填滿**：依夜序 1→N 逐天處理。當天景點（含上一夜 cascade 進來的）依「離當晚飯店」近→遠排隊，平手用 placeId 字典序；逐一塞入，**若塞下去會超過當天預算，就把該景點往後溢到下一夜**（forward cascade，不回頭平衡）。
   - **最後一夜承接所有剩餘**（不擋預算；可能超量 → 由既有 `outsideHours`/`lateExit` 警告）。
   - 結果：**前面的天先被排滿、溢出往後堆、最後一天吸收剩餘——刻意不平分**。決定性（同輸入同輸出）。
...
**(b) 緊預算（3h/天 = 每天約 2 個景點，逼出 cascade）—— 驗證「不平分」：**
```
分群（累進填滿、溢到隔天、不平分）：
   夜1 (台場Hotel)  3h00： 台場海濱公園 / 富士電視台          ← 排滿
   夜2 (淺草Hotel)  3h00： 雷門/淺草寺 / 東京晴空塔            ← 排滿
   夜3 (新宿Hotel)  7h30： 新宿御苑 / 明治神宮 / 澀谷十字路口 / 上野公園 / 豐洲市場  ← 最後一天吸收溢出
```

**結論：Approach B + 累進填滿在真實座標上可行**——地理分區合理、端點固定路線正確；緊預算時**前面的天先排滿、溢出往後堆、最後一天承接剩餘，明確不平分**（符合使用者指定）。
