...
| **C3** | 共享候選池（append-only 口袋名單）| **SHIPPED** (merged to main `95a408f`; live Supabase verify pending keys) | C1 |
| **C4** | 候選池依地理分到各天（帶 `←` 加入箭頭）| **DONE** (branch: `lane/c4-candidate-arrange`; code-first, live verify pending keys) | C3 |
